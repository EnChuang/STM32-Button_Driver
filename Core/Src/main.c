...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define	ReadA0	HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_0)
#define	ReadA1	HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_1)
/* USER CODE END PD */
...
int main(void)
{
    ...
    MX_USART3_UART_Init();
    /* USER CODE BEGIN 2 */
    bsp_Init();																								//外部設備(Board support package)初始化

    HAL_TIM_Base_Start_IT(&htim3);					//按鈕10ms掃描一次，timer3配置10ms中斷一次
    /* USER CODE END 2 */
